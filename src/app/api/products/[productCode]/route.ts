...
export async function GET(
  ...
  const rl = isLoadTest(request) ? { success: true, remaining: 100 } : await rateLimit(ip, 'rl:product-detail', 100, 60);
  if (!rl.success) {
  ...
  if (!productCode) {
    const duration = Date.now() - startTime;
    log.warn({ status: 400, duration, ...(testRun !== undefined ? { testRun } : {}) }, 'Invalid product code');
  ...
  if (!result.success) {
    ...
    if (status === 404) {
      log.debug({ productCode, status, duration: Date.now() - startTime, ...(testRun !== undefined ? { testRun } : {}) }, 'Product not found');
    } else {
      log.error(
        { productCode, error: result.error.message, status, duration: Date.now() - startTime, ...(testRun !== undefined ? { testRun } : {}) },
  ...
  const duration = Date.now() - startTime;
  log.info({ productCode, status: 200, duration, ...(testRun !== undefined ? { testRun } : {}) }, 'Product detail request completed');
